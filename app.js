...
async function startCamera() {
  ...
  try {
    ...
    if (errorEl) { errorEl.style.display = 'block'; errorEl.textContent = 'לא ניתן לגשת למצלמה. נסה להעלות תמונה.'; }
...
async function analyzeImageWithAI(imageData) {
  ...
  resultEl.innerHTML = `<div class="analyzing-spinner"><div class="spinner"></div><p>מנתח עם AI...</p></div>`;
  try {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          ...
          content: [
            ...
            { type: "text", text: `אתה מומחה תזונה. נתח את התמונה וזהה את האוכל/מנה.
החזר JSON בלבד:
{"name":"שם בעברית","quantity":"כמות משוערת","calories":מספר,"protein":מספר,"carbs":מספר,"fat":מספר,"confidence":"high/medium/low","notes":"הערה קצרה"}
אם אין אוכל: {"error":"לא זוהה אוכל"}` }
          ]
        }]
      })
    });
    ...
    if (parsed.error) { resultEl.innerHTML = `<div class="no-results">⚠️ ${parsed.error}</div>`; return; }
    renderCameraResult(parsed);
  } catch (err) {
    resultEl.innerHTML = `<div class="no-results">❌ שגיאה בניתוח. נסה שוב.</div>`;
  }
}

function renderCameraResult(food) {
  const resultEl = document.getElementById('camera-result');
  const confLabel = { high: '✅ בטוח', medium: '🟡 בינוני', low: '🔴 לא בטוח' };
  resultEl.innerHTML = `
    <div class="camera-result-inner">
      <div class="camera-result-header">
        <strong>${food.name}</strong>
        <span class="conf-badge">${confLabel[food.confidence] || ''}</span>
      </div>
      <div class="camera-result-qty">כמות: ${food.quantity}</div>
      <div class="camera-macros">
        <div class="cm-item"><span class="cm-val" style="color:var(--accent2)">${Math.round(food.calories)}</span><small>קלוריות</small></div>
        <div class="cm-item"><span class="cm-val" style="color:#EF4444">${Math.round(food.protein)}g</span><small>חלבון</small></div>
        <div class="cm-item"><span class="cm-val" style="color:#3B82F6">${Math.round(food.carbs)}g</span><small>פחמימות</small></div>
        <div class="cm-item"><span class="cm-val" style="color:#F59E0B">${Math.round(food.fat)}g</span><small>שומן</small></div>
      </div>
      ${food.notes ? `<div class="camera-notes">💡 ${food.notes}</div>` : ''}
      <div class="camera-result-btns">
        <button class="btn-primary" onclick='addCameraFood(${JSON.stringify(food).replace(/'/g,"&#39;")})'>✅ הוסף ליומן</button>
        <button class="btn-secondary" onclick='openEditCameraFood(${JSON.stringify(food).replace(/'/g,"&#39;")})'>✏️ ערוך</button>
      </div>
    </div>`;
}

function addCameraFood(food) {
  const item = {
    id: "log_" + Date.now(),
    name: food.name,
    emoji: "📷",
    amount: 100,
    unitLabel: food.quantity,
...
function openEditCameraFood(food) {
  editingItemId = null;
  document.getElementById('edit-name').value = food.name;
  document.getElementById('edit-cal').value = Math.round(food.calories);
  ...
  closeCameraModal();
  // Override save to add new item
  const btn = document.getElementById('btn-save-edit');
  btn.onclick = () => {
    const item = {
      id: "log_" + Date.now(),
      name: document.getElementById('edit-name').value.trim() || food.name,
      emoji: "📷",
      amount: parseFloat(document.getElementById('edit-amount').value) || 100,
      unitLabel: document.getElementById('edit-amount').value + "ג'",
      cal: parseFloat(document.getElementById('edit-cal').value) || 0,
...
async function startBarcodeScanner() {
  ...
  if (errorEl) errorEl.style.display = 'none';
  document.getElementById('manual-barcode-wrap').style.display = 'none';
  ...
  // Fallback
  if (errorEl) { errorEl.style.display = 'block'; errorEl.textContent = 'הסורק האוטומטי אינו נתמך בדפדפן זה.'; }
...
function closeBarcodeModal() {
  ...
  document.getElementById('manual-barcode-wrap').style.display = 'none';
  if (document.getElementById('barcode-error')) document.getElementById('barcode-error').style.display = 'none';
}

async function lookupBarcode(barcode) {
  const resultEl = document.getElementById('barcode-result');
  resultEl.style.display = 'block';
  resultEl.innerHTML = `<div class="analyzing-spinner"><div class="spinner"></div><p>מחפש מוצר ${barcode}...</p></div>`;
  try {
    ...
    if (data.status === 1 && data.product) {
      ...
      resultEl.innerHTML = `<div class="no-results">⚠️ מוצר לא נמצא (${barcode})<br><button class="btn-secondary" style="margin-top:10px" onclick="lookupBarcodeAI('${barcode}')">🤖 נסה AI</button></div>`;
    }
  } catch(e) {
    resultEl.innerHTML = `<div class="no-results">❌ שגיאת חיבור</div>`;
  }
}

function lookupManualBarcode() {
  const val = document.getElementById('barcode-manual-input').value.trim();
  if (!val || val.length < 6) { showToast('הזן ברקוד תקין'); return; }
  lookupBarcode(val);
}

async function lookupBarcodeAI(barcode) {
  const resultEl = document.getElementById('barcode-result');
  resultEl.innerHTML = `<div class="analyzing-spinner"><div class="spinner"></div><p>שואל AI...</p></div>`;
  try {
    const res = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        messages: [{ role: "user", content: `ברקוד: ${barcode}. האם אתה מכיר מוצר מזון זה? תן ערכים ל-100 גרם. JSON בלבד:
{"name":"שם","brand":"מותג","calories":X,"protein":X,"carbs":X,"fat":X}
אם לא מכיר: {"error":"לא נמצא"}` }]
      })
    });
    const data = await res.json();
    const parsed = JSON.parse(data.content?.map(b => b.text || '').join('').replace(/```json|```/g, '').trim());
    if (parsed.error) { resultEl.innerHTML = `<div class="no-results">מוצר לא נמצא</div>`; }
    else renderBarcodeProduct({ ...parsed, image: null });
  } catch(e) { resultEl.innerHTML = `<div class="no-results">❌ שגיאה</div>`; }
}
...
function renderBarcodeProduct(product) {
  ...
  resultEl.innerHTML = `
    <div class="barcode-product-card">
      ${product.image ? `<img src="${product.image}" class="barcode-product-img" alt="">` : '<div class="barcode-no-img">📦</div>'}
      <div class="barcode-product-info">
        <div class="barcode-product-name">${product.name}</div>
        ${product.brand ? `<div class="barcode-product-brand">${product.brand}</div>` : ''}
        <div class="barcode-per100">ערכים ל-100 גרם:</div>
      </div>
      <div class="camera-macros">
        <div class="cm-item"><span class="cm-val" style="color:var(--accent2)">${Math.round(product.calories)}</span><small>קלוריות</small></div>
        <div class="cm-item"><span class="cm-val" style="color:#EF4444">${Math.round(product.protein)}g</span><small>חלבון</small></div>
        <div class="cm-item"><span class="cm-val" style="color:#3B82F6">${Math.round(product.carbs)}g</span><small>פחמימות</small></div>
        <div class="cm-item"><span class="cm-val" style="color:#F59E0B">${Math.round(product.fat)}g</span><small>שומן</small></div>
      </div>
      <div class="barcode-portion-row">
        <label>כמות (גרם):</label>
        <input type="number" id="barcode-grams" value="100" min="1" max="2000" class="barcode-grams-input">
      </div>
      <button class="btn-primary btn-full" onclick="addBarcodeProductToLog('${product.name.replace(/'/g,"\\'")}',${product.calories},${product.protein},${product.carbs},${product.fat})">✅ הוסף ליומן</button>
    </div>`;
}

function addBarcodeProductToLog(name, cal, protein, carbs, fat) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById('barcode-modal')?.addEventListener('click', (e) => { if (e.target.id === 'barcode-modal') closeBarcodeModal(); });
  document.getElementById('camera-modal')?.addEventListener('click', (e) => { if (e.target.id === 'camera-modal') closeCameraModal(); });
  document.getElementById('barcode-manual-input')?.addEventListener('keypress', (e) => { if (e.key === 'Enter') lookupManualBarcode(); });
  document.getElementById('btn-close-barcode')?.addEventListener('click', closeBarcodeModal);
});
